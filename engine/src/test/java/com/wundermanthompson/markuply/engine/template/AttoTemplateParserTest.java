package com.wundermanthompson.markuply.engine.template;

import com.wundermanthompson.markuply.engine.template.graph.node.*;
...
import static com.wundermanthompson.markuply.engine.template.graph.node.ComponentSectionFragment.DEFAULT_SECTION_NAME;
...
public class AttoTemplateParserTest {
  ...
  @Test
  void fullStaticPage() {
    AttoTemplateParser parser = AttoTemplateParser.instance();
  ...
  void simpleNestedComponentPage() {
    ...
    assertStaticFragment(section.getChildren().get(0));
    assertComponent(section.getChildren().get(1), "hello", "World");
    assertStaticFragment(section.getChildren().get(2));
  }
